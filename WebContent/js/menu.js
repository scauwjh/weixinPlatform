...
$(document).ready(function(){
  ...
  $("#dialog-add").dialog({			//添加菜单节点
  	autoOpen:false, height:300, width:500, modal:true, draggable:true, buttons:{ 
  		提交:function(){  
			var temp_id = new_id();
			...
			for(var i=0; i<json.button.length; i++){
				if(json.button[i].id == place){
					...
					json.button[i].sub_button.push({"id":temp_id, "type":"view", "name":temp_name, "url":"#"});
				}
			}
			init();
  ...
  $("#dialog-del").dialog({ 		//删除节点
  	autoOpen:false, height:220, width:300, modal:true, draggable:false, buttons:{ 
  		提交:function(){
  ...
  $("#dialog-mod").dialog({ 		//修改菜单名称
  	autoOpen:false, height:300, width:500, modal:true, draggable:false, buttons:{ 
  		提交:function(){  
  ...
  $("#dialog-form").dialog({
  	autoOpen:false, height:300, width:500, modal:true, draggable:false, buttons:{ 
  		提交:function(){
			...
			var temp_id = new_id();
			json.button.push({"id":temp_id, "type":"view", "name":temp_name, "url":"#"});
			init();
			//var req = JSON.stringify(json);
			//alert(req);
  ...
  $("#dialog-lang").dialog({ 
  	autoOpen:false, height:500, width:700, modal:true, draggable:false, buttons:{ 
  		提交:function(){  
  			alert("提交什么什么的"); } 
  		,返回:function(){
...
function open_dialog(temp,temp_id){
	...
	$('.ui-dialog-titlebar').css({'background-color': 'rgb(200,200,200)','border':'none'});
}
		...
		for(var i=0; i<json.button.length; i++){
			if(json.button[i].id == tag_id){
				if(json.button[i].type == "click"){
					...
					json.button[i].message={"type":"msg","msg":tag_msg};
				...
				for(var j=0; j<json.button[i].sub_button.length; j++){
					if(json.button[i].sub_button[j].id == tag_id){
						if(json.button[i].sub_button[j].type == "click"){
							...
							json.button[i].sub_button[j].message={"type":"msg","msg":tag_msg};
